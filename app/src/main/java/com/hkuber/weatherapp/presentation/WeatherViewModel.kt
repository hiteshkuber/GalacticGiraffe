    ...
    fun loadWeatherInfo() {
        viewModelScope.launch {
            ...
            locationTracker.getCurrentLocation()?.let {
                when(val result = repository.getWeatherData(latitude = it.latitude, longitude = it.longitude)) {
                    is Resource.Success -> {
                        state = state.copy(
                ...
                state = state.copy(
                    isLoading = false,
                    error = "Couldn't retrieve location. MAke sure to grant required permissions."
                )
            }
        }
    }
